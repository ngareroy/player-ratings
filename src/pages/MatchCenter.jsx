...
import { subscribeMatches, subscribeTeams, saveMatch, removeMatch, subscribeClubSettings } from '../firebase'
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999, padding: 16 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: "linear-gradient(145deg,#1a1a2e,#16213e)", borderRadius: 16, width: "100%", maxWidth: 480, maxHeight: "90vh", overflow: "auto", boxShadow: "0 16px 64px rgba(0,0,0,0.6)", border: "1px solid rgba(255,255,255,0.1)", padding: 24 }}>
        ...
        {/* Score */}
        <div style={{ marginBottom: 14 }}>
          <label style={{ color: "rgba(255,255,255,0.4)", fontSize: 11, fontWeight: 600, display: "block", marginBottom: 6 }}>SCORE (leave blank for upcoming matches)</label>
...
export default function MatchCenter() {
  ...
  const past = filtered.filter(m => m.date < todayStr || (m.goalsFor !== null && m.goalsFor !== undefined))
      ...
      <div key={m.id} style={{ padding: "14px 20px", borderBottom: "1px solid rgba(255,255,255,0.03)", display: "flex", alignItems: "center", gap: 12, cursor: "pointer", transition: "background 0.2s" }}
        ...
        <div style={{ flex: 1 }}>
          <div style={{ color: "#fff", fontSize: 14, fontWeight: 700 }}>
            {hasScore ? (
              <><span style={{ color: resultColor[result] || "#fff" }}>{club.clubName} {m.goalsFor}</span> <span style={{ color: "rgba(255,255,255,0.2)" }}>—</span> <span>{m.goalsAgainst} {m.opponent}</span></>
            ) : (
      ...
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <h1 style={{ color: "#fff", fontSize: 22, fontWeight: 800, margin: 0, letterSpacing: 1 }}>{club.clubName || "Hub FC"} MATCH CENTER</h1>
          </div>
