...
import {
    ATTRS, GK_ATTRS, CAT_ORDER, CAT_FORMULAS, calcCategories, calcGkCategory,
    calcOverall, calcBestRating, calcAllPositionRatings, calcAge
} from './utils'
...
export async function generatePlayerReport(player, clubSettings, teamNames, matchStats, history, attendanceData) {
    ...
    if (attendanceData && attendanceData.total > 0) {
        ...
        const attBoxW = 30
        const attItems = [
            { label: "Sessions", value: attendanceData.total },
            { label: "Present", value: attendanceData.present },
            { label: "Late", value: attendanceData.late },
            { label: "Absent", value: attendanceData.absent },
            { label: "Rate", value: Math.round(attendanceData.rate) + "%" },
        ]

        attItems.forEach((s, i) => {
            const x = margin + i * (attBoxW + 6)
            doc.setFillColor(25, 30, 50)
            doc.roundedRect(x, y, attBoxW, 13, 2, 2, 'F')
            doc.setFontSize(11)
            doc.setFont("helvetica", "bold")
            const [ar, ag, ab] = s.label === "Rate" ? (attendanceData.rate >= 90 ? [46, 204, 64] : attendanceData.rate >= 75 ? [123, 199, 77] : attendanceData.rate >= 60 ? [232, 185, 48] : [231, 76, 60]) : [255, 255, 255]
            doc.setTextColor(ar, ag, ab)
            doc.text(String(s.value), x + 3, y + 8)
            doc.setFontSize(5)
            doc.setTextColor(120, 120, 120)
            doc.text(s.label.toUpperCase(), x + 3, y + 12)
        })
        y += 20
    }
